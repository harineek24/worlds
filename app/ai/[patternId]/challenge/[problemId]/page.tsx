...
import { getAiTopic } from "@/content/ai-eng"
...
export default function AiChallengePage({
  ...
  const pattern = getAiTopic(patternId)
  if (!pattern) notFound()
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex flex-col gap-8 px-10 py-8">
        ...
        <TwoColumn
          ...
          right={
            <div className="flex flex-col gap-3">
              <p className="text-[#a0845c] text-xs uppercase tracking-widest">Which topic?</p>
              ...
              {submitted ? (
                ...
                <div className="mt-3 pt-3 border-t border-[#2a1f0e]">
                  <p className="text-[#4a3520] font-mono text-xs italic">Topic revealed on submit</p>
                </div>
              )}
            </div>
          }
        ...
        {/* Explanation blanks — render explanationBlanks directly, no text matching needed */}
        {showExplanation && (
          <TwoColumn
                          ...
                          <span key={j} className="inline-flex items-baseline gap-1 flex-wrap">
                            ...
                            {j < parts.length - 1 && (
                              <input
                                ...
                                disabled={submitted}
                                placeholder="___"
                                className={`w-32 px-2 py-0.5 rounded text-center font-mono text-xs outline-none border transition-colors ${
                                  !submitted
        ...
        <div className="flex items-center justify-between border-t border-[#2a1f0e] pt-6 pb-8">
          ...
          <div className="flex gap-3">
            <Link
              href={`/ai/${patternId}/study/${problemId}`}
            ...
            {!submitted && mode !== "code" && (
              <Link
                href={`/ai/${patternId}/challenge/${problemId}?mode=code`}
                className="px-5 py-3 border border-[#5c3d1e] text-[#a0845c] hover:text-[#f5e6c8] font-mono text-sm rounded transition-colors"
              >
                Switch to Code
              </Link>
            ...
            {!submitted && mode !== "explanation" && (
              <Link
                href={`/ai/${patternId}/challenge/${problemId}?mode=explanation`}
                className="px-5 py-3 border border-[#5c3d1e] text-[#a0845c] hover:text-[#f5e6c8] font-mono text-sm rounded transition-colors"
              >
                Switch to Reasoning
              </Link>
            ...
            {!submitted ? (
              ...
              <Link
                href={`/ai/${patternId}/study/${nextProblem.id}`}
                className="px-8 py-3 bg-[#5c3d1e] hover:bg-[#7a5230] text-[#f5e6c8] font-mono text-sm rounded transition-colors"
              >
                Next Problem →
              </Link>
            ) : (
              <span className="px-8 py-3 text-[#4a3520] font-mono text-sm">Topic Complete</span>
            )}
          </div>
        </div>
